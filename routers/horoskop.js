...
const ramalanService = require("../helpers/axiosService");
...
router.get("/besok/:query", async (req, res) => {
  const query = req.params.query;
  const url = `https://www.ramalan-harian.com/ramalan-bintang/besok/${query}.htm`;
  // belom
  try {
    const response = await ramalanService(url);
    const $ = cheerio.load(response.data);
...
router.get("/lusa/:query", async (req, res) => {
  const query = req.params.query;
  const url = `https://www.ramalan-harian.com/ramalan-bintang/lusa/${query}.htm`;
  // belom
  try {
    const response = await ramalanService(url);
    const $ = cheerio.load(response.data);
